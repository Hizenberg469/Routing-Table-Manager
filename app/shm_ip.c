...
/*
* Stores IP address in newly created shared memory region
* corresponding to its key, which is a MAC address. Returns
* the size of created shm on success otherwise -1 on failure.
*/
...
/*
* Get the IP address corresponding to the MAC address
* from the shared region that the server created.
* Returns number of bytes on success else -1 on
* failure.
*/

int get_IP(const char* mac, char* ip) {

	int shm_fd = shm_open(mac,
		O_CREAT | O_RDONLY,
		0660);

	if (shm_fd == -1) {
		printf("Couldn't open shared memory for MAC %s - IP %s pair\n",
			mac,
			ip);
		return -1;
	}
